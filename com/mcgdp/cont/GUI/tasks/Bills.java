...
public class Bills extends JFrame implements ActionListener {
	...
	public Bills() {
		...
		System.out.println("Bills [INFO]: Bills iniciada.");
	}
	
	// Inicialización de componentes
	private void initialize() {
		System.out.println("Bills [INFO]: Iniciando ventana de Bills...");
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			System.out.println("Bills [FATAL]: Error Fatal: " + err.getMessage() + " " + 					// Errores
					err.getCause());
			err.printStackTrace();
	...
	private void construirTableReview() {																	// Tabla de datos
		System.out.println("Bills [INFO]: Creando tabla 'En revisión'...");									// En revisión
		...
		if(data.length != 0) {																			 	// Si existen datos
			...
			System.out.println("Bills [INFO]: Tabla creada: 'En revisión...'");
		}
		else {																								// Si no existen datos
			lblNoReview = new JLabel("No hay Cuentas de Pago en Revisión");									// Propiedades de la etiqueta
			lblNoReview.setFont(new Font("Tahoma", Font.PLAIN, 25));										// No hay en proceso
	...
	private void construirTableApproved() {																	// Tabla de datos
		System.out.println("Bills [INFO]: Creando tabla 'Aprobados'...");									// Aprobados
		...
		if(data.length != 0) {																			 	// Si existen datos
			...
			System.out.println("Bills [INFO]: Tabla creada: 'Aprobados'");
		}
		else {																								// Si no existen datos
			lblNoApproved = new JLabel("No hay Cuentas de Pago Aprobadas");									// Propiedades de la etiqueta
			lblNoApproved.setFont(new Font("Tahoma", Font.PLAIN, 25));										// No hay en proceso
			lblNoApproved.setHorizontalAlignment(SwingConstants.CENTER);
			scrollApproved.setViewportView(lblNoApproved);
			System.out.println("Bills [INFO]: Tabla 'Aprobados' no creada por falta de datos");
		}
	}
	...
	private void exit() {																					// Cerrando ventana
		if(JOptionPane.showConfirmDialog(null,																// Ventana de confirmación
				"¿Seguro que deseas salir de Cuentas por Pagar?", "Auxiliar Contable",
				JOptionPane.YES_NO_OPTION) == JOptionPane.YES_NO_OPTION) {
				System.out.println("Bills [INFO]: Saliendo de Cuentas por Pagar...");
				inicio = new Index();
